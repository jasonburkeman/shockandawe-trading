...
const Playback = ({ state, setState }) => {
    // ⚠️ SECURITY: Keep this empty for GitHub. Use .env or input box for local dev.
    const HARDCODED_KEY = ""; 
...
const StrategyRoom = ({ state, setState }) => {
    ...
    const handleSendMessage = async () => {
        ...
        const context = imageBase64 ? "Refer to the chart provided." : "No chart uploaded yet. Provide general trading advice based on standard price action.";
        const prompt = `You are an expert trader. User Question: "${msg}". ${context}
        CRITICAL: Keep answer clean, human readable, NO BOLDING (**), NO MARKDOWN headers.`;
            ...
            <Card className="lg:w-1/3 flex flex-col h-full bg-slate-900">
                ...
                <div className="p-3 border-t border-slate-800 flex gap-2">
                    <input 
                        ...
                        disabled={isAnalyzing}
                    />
                    <button onClick={handleSendMessage} disabled={isAnalyzing} className="bg-indigo-600 hover:bg-indigo-500 text-white p-2 rounded">
                        <Send size={16} />
                    </button>
                </div>
            </Card>
...
const calculateMetrics = (trades, balance, overhead, selectedAccount) => {
    // FILTER TRADES FOR METRICS
    const activeTrades = selectedAccount === 'All' 
        ? trades 
        : trades.filter(t => (t.account || 'Main') === selectedAccount);
    ...
    // 11. Overall Vibe
    if (netPnL > 0 && winRate > 50) insights.push(`🚀 Solid Performance: ${winRate.toFixed(0)}% WR and Green.`);
    ...
    SHOCK AND AWE TRADING REPORT (${selectedAccount})
    ----------------------------
    Net PnL: $${netPnL.toFixed(2)}
    Biz Net: $${bizNet.toFixed(2)} (Overhead: -$${totalOverhead})
    ...
    return {
        ...
        businessNetPnL: bizNet.toFixed(2),
        totalOverhead,
        expectancy: (expectancy || 0).toFixed(2),
...
export default function TradingJournal() {
    ...
    const [accounts, setAccounts] = useState(() => JSON.parse(localStorage.getItem('journal_accounts')) || ['Main']);
    ...
    // Persist Accounts
    useEffect(() => {
        localStorage.setItem('journal_accounts', JSON.stringify(accounts));
    }, [accounts]);
    ...
    const [newTrade, setNewTrade] = useState({
        ...
        account: 'Main' // Default account for new trade
    });

    // Update new trade default account when accounts change
    useEffect(() => {
        if (!newTrade.account && accounts.length > 0) {
             setNewTrade(prev => ({ ...prev, account: accounts[0] }));
        }
    }, [accounts]);
    ...
    // FILTER TRADES FOR METRICS CALCULATION
    ...
    const saveTrade = () => {
        ...
        const tradeData = { 
            ...
            fees: fees.toFixed(2),
            // Ensure account is set. If 'All' is selected, default to first account.
            account: newTrade.account || (selectedAccount === 'All' ? accounts[0] : selectedAccount)
        };
        ...
        setNewTrade({ 
            ...
            account: selectedAccount === 'All' ? accounts[0] : selectedAccount
        });
    };
    ...
    const parseTopstep = (text) => {
        ...
        // Determine target account for import
        ...
        for(let i=1; i<lines.length; i++) {
            ...
            newTrades.push({
                ...
                setup: 'Imported',
                account: targetAccount // Assign to account
            });
        }
        setTrades([...trades, ...newTrades]);
        alert(`Imported ${newTrades.length} trades to account: ${targetAccount}`);
    };

    const parseNinja = (text) => {
        const lines = text.split('\n');
        const headers = lines[0].split(',').map(h => h.trim().replace(/"/g, ''));
        ...
        for(let i=1; i<lines.length; i++) {
             ...
             newTrades.push({ id: Date.now()+i, date: new Date().toISOString().split('T')[0], time: "12:00", ticker: row[idxInstr], direction: 'Long', size: 1, pnl: pnl.toFixed(2), fees: "0.00", notes: "Ninja Import", setup: 'Imported', account: targetAccount });
        }
        setTrades([...trades, ...newTrades]);
    };

    const parseTakeProfit = (text) => {
        const lines = text.split('\n').filter(line => line.trim() !== '');
        if (lines.length < 2) return;
        const headers = lines[0].split(',').map(h => h.trim().replace(/"/g, '').toLowerCase());

        // Mappings for Tradovate / TPT
        const colMap = {
            ticker: headers.findIndex(h => h.includes('symbol') || h.includes('contract') || h.includes('instrument')),
            pnl: headers.findIndex(h => h.includes('p&l') || h.includes('profit') || h.includes('pl') || h.includes('realized')),
            date: headers.findIndex(h => h.includes('time') || h.includes('date') || h.includes('filled')),
            side: headers.findIndex(h => h.includes('side') || h.includes('b/s') || h.includes('direction')),
            size: headers.findIndex(h => h.includes('qty') || h.includes('quantity') || h.includes('size')),
            fee: headers.findIndex(h => h.includes('fee') || h.includes('commission'))
        };

        if (colMap.pnl === -1) {
            alert("Could not find P&L column. Please ensure your CSV has a Profit/Loss column.");
            return;
        }
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            const ticker = colMap.ticker > -1 ? row[colMap.ticker] : 'Unknown';
            let pnl = parseFloat(row[colMap.pnl].replace(/[^0-9.-]/g, ''));
            if (row[colMap.pnl].includes('(')) pnl = -Math.abs(pnl); // Handle ($50) format

            let fees = 0;
            const size = colMap.size > -1 ? parseFloat(row[colMap.size]) || 1 : 1;
            
            if (colMap.fee > -1) {
                fees = parseFloat(row[colMap.fee].replace(/[^0-9.-]/g, ''));
            } else {
                fees = size * overhead.commRate;
            }
            ...
            const direction = colMap.side > -1 ? row[colMap.side] : (pnl >= 0 ? 'Long' : 'Short'); // Guess if unknown
    ...
    const exportCSV = () => { /* reuse */
        const headers = ["Date", "Account", "Ticker", "PnL", "Strategy", "Mistake", "Duration", "Fees"];
        const csvContent = "data:text/csv;charset=utf-8," + headers.join(",") + "\n" + trades.map(t => [t.date, t.account, t.ticker, t.pnl, t.setup, t.mistake, t.duration, t.fees].join(",")).join("\n");
    ...
    const addAccount = () => {
        const name = prompt("Enter new account name (e.g. 'Apex 50k'):");
    ...
    // Filtered trades for list view
    const displayedTrades = useMemo(() => {
        if (selectedAccount === 'All') return trades;
        return trades.filter(t => (t.account || 'Main') === selectedAccount);
            ...
            <div className="flex items-center gap-4">
                ...
                <div className="flex items-center bg-slate-900 border border-slate-700 rounded px-2 py-1">
                    ...
                    <select 
                        ...
                        className="bg-transparent text-sm text-white outline-none mr-2"
                    >
                        ...
                        <CardHeader>
                            <div className="flex justify-between items-center">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button onClick={() => handleImportClick('takeprofit')} className="flex items-center gap-2 bg-amber-600 hover:bg-amber-500 px-3 py-1 rounded text-xs transition-colors text-white font-medium">
                                    <Upload size={14} /> TP Trader
                                    </button>
                     ...
                     <div className="overflow-x-auto">
                         <table className="w-full text-sm text-left text-slate-400">
                             <thead className="text-xs uppercase bg-slate-900 border-b border-slate-800">
                                 <tr><th className="px-4 py-2">Date</th><th className="px-4 py-2">Account</th><th className="px-4 py-2">Ticker</th><th className="px-4 py-2">Strategy</th><th className="px-4 py-2">P&L</th><th className="px-4 py-2 text-right">Action</th></tr>
                             </thead>
                             <tbody>
